        ...
        cachedWallet.clear();
        {
            ...
            for(std::map<uint256, CWalletTx>::iterator it = wallet->mapWallet.begin(); it != wallet->mapWallet.end(); ++it)
            {
                ...
                if(KernelRecord::showTransaction(it->second))
                    Q_FOREACH(const KernelRecord& kr, txList) {
                        if(!kr.spent) {
    ...
    void updateWallet(const uint256 &hash, int status)
    {
        LogPrintf("minting updateWallet %s %i\n", hash.ToString().c_str(), status);
        {
            ...
            case CT_NEW:
                ...
                if(showTransaction)
                {
                    ...
                    if(toInsert.size() != 0) /* only if something to insert */
                    {
                        ...
                        Q_FOREACH(const KernelRecord &rec, toInsert)
                        {
                            if(!rec.spent)
                            {
            ...
            case CT_UPDATED:
                ...
                if(!toCheck.empty())
                {
                    Q_FOREACH(const KernelRecord &rec, toCheck)
                    {
                        if(rec.spent)
                        {
    ...
    {
        {
            ...
            {
                return TransactionDesc::toHTML(wallet, mi->second, NULL, 1);
            }
        }
        return QString("");
    }
    ...
    switch(role)
    {
      ...
      case Qt::EditRole:
        ...
        case TxHash:
            return formatTxHash(rec);
        case Age:
            return QVariant::fromValue(rec->getAge());
        case CoinDay:
            return QVariant::fromValue(rec->coinAge);
        case Balance:
            return QVariant::fromValue(rec->nValue);
        case MintProbability:
            return getDayToMint(rec);
        }
        break;
      case Qt::BackgroundColorRole:
        int minAge = Params().GetConsensus().nStakeMinAge;
        int maxAge = minAge * 6;
...
{
    const CBlockIndex *p = GetLastBlockIndex(pindexBestHeader, true);
...
QString MintingTableModel::formatTxAge(const KernelRecord *wtx) const
{
    ...
    return QDateTime::fromTime_t(nAge).toString("hh mm ss");
}

QString MintingTableModel::formatTxBalance(const KernelRecord *wtx) const
{
    return KoreUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), wtx->nValue);
}
...
{
    // Q_EMIT dataChanged to update Balance column with the current unit
