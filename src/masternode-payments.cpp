...
//             masternodePayment = GetMasternodeFundPayment(ncredit, nstakedBallance, pindexPrev);
//             txNew.vout.resize(3);
...
//         } else {
//             masternodePayment = GetMasternodeFundPayment(pindexPrev->nHeight, blockReward, 0);
...
//     CAmount masternodePayment = GetMasternodeFundPayment_Legacy(pindexPrev->nHeight, blockValue);
...
//                 Misbehaving(pfrom->GetId(), 3, "Peer already asked for the list");
//                 return;
//             }
//         }
...
//                 Misbehaving(pfrom->GetId(), 20, "Winner signature invalid");
...
//     CAmount requiredMasternodePayment = UseLegacyCode(nBlockHeight) ? GetMasternodeFundPayment_Legacy(nBlockHeight, nReward) : GetMasternodeFundPayment(nBlockHeight, nReward, pindexPrev);
