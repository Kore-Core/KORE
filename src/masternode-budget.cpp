...
bool IsBudgetCollateralValid(uint256 nTxCollateralHash, uint256 nExpectedHash, std::string& strError, int64_t& nTime, int& nConf)
{
    ...
    */
    ...
    if (nBlockHash != uint256S(itostr(0))) {
...
void CBudgetManager::SubmitFinalBudget()
{
    ...
    uint256 hhash;
    CFinalizedBudgetBroadcast tempBudget(strBudgetName, nBlockStart, vecTxBudgetPayments, hhash);
...
struct sortProposalsByVotes {
    bool operator()(const std::pair<CBudgetProposal*, int>& left, const std::pair<CBudgetProposal*, int>& right)
    {
        ...
        return (right.first->nFeeTXHash < left.first->nFeeTXHash);
    }
};
    ...
    if (masternodeSync.IsSynced()) {
        ...
        BOOST_FOREACH (CNode* pnode, vNodes)
            if (pnode->nVersion >= MIN_BUDGET_PEER_PROTO_VERSION)
                Sync(pnode, uint256S(itostr(0)), true);
...
void CBudgetManager::Sync(CNode* pfrom, uint256 nProp, bool fPartial)
{
    ...
    */
    ...
    while (it1 != mapSeenMasternodeBudgetProposals.end()) {
        CBudgetProposal* pbudgetProposal = FindProposal((*it1).first);
        if (pbudgetProposal && pbudgetProposal->fValid && (nProp == uint256S(itostr(0)) || (*it1).first == nProp)) {
    ...
    while (it3 != mapSeenFinalizedBudgets.end()) {
        CFinalizedBudget* pfinalizedBudget = FindFinalizedBudget((*it3).first);
        if (pfinalizedBudget && pfinalizedBudget->fValid && (nProp == uint256S(itostr(0)) || (*it3).first == nProp)) {
...
{
    vin = CTxIn();
    nProposalHash = uint256S(itostr(0));
    nVote = VOTE_ABSTAIN;
    nTime = 0;
    fValid = true;
    fSynced = false;
...
bool CFinalizedBudget::IsValid(std::string& strError, bool fCheckCollateral)
{
    ...
    if (nFeeTXHash == uint256S(itostr(0))) {
