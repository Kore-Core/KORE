...
const std::string KoreGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString KoreGUI::DEFAULT_WALLET = "~Default";

KoreGUI::KoreGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
    ...
    platformStyle(platformStyle)
{
    ...
    QLabel *labelUsdlabel = new QLabel("KORE/USD");
...
void KoreGUI::on_opacitySpinBox_valueChanged(int i)
{
...
void KoreGUI::createActions()
{
    ...
    aboutAction = new QAction(platformStyle->TextColorIcon(":/icons/about"), tr("&About Kore"), this);
...
void KoreGUI::engageDisengageMining(int cores)
{
    isMiningEngaged = false;
    if (cores > 0)
        isMiningEngaged = true;

    GenerateKores(isMiningEngaged, cores, Params());

}

void KoreGUI::createToolBars()
{
...
void KoreGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool KoreGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool KoreGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void KoreGUI::setWalletActionsEnabled(bool enabled)
{
...
void KoreGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void KoreGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void KoreGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void KoreGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void KoreGUI::showHelpMessageClicked()
{
...
void KoreGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void KoreGUI::gotoMintingPage()
{
    mintingAction->setChecked(true);
    if (walletFrame) walletFrame->gotoMintingPage();
}

void KoreGUI::gotoHistoryPage()
{
...
void KoreGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void KoreGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void KoreGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

void KoreGUI::gotoTradingPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoTradingPage();
}

void KoreGUI::gotoMultiSendDialog()
{
    multiSendAction->setChecked(true);
    if (walletFrame)
        walletFrame->gotoMultiSendDialog();
}
void KoreGUI::gotoBlockExplorerPage()
{
...
void KoreGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
...
void KoreGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress)
{
    if(!clientModel)
        return;
...
void KoreGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
...
void KoreGUI::changeEvent(QEvent *e)
{
...
void KoreGUI::closeEvent(QCloseEvent *event)
{
    if (isMiningEngaged)
...
void KoreGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void KoreGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(KoreUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void KoreGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void KoreGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool KoreGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool KoreGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void KoreGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void KoreGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void KoreGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void KoreGUI::detectShutdown()
{
...
void KoreGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
static bool ThreadSafeMessageBox(KoreGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void KoreGUI::subscribeToCoreSignals()
{
    // Connect signals to client
    uiInterface.ThreadSafeMessageBox.connect(boost::bind(ThreadSafeMessageBox, this, _1, _2, _3));
}

void KoreGUI::unsubscribeFromCoreSignals()
{
...
void KoreGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<KoreUnits::Unit> units = KoreUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const KoreUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(KoreUnits::name(unit)));
    }
...
{
    menu = new QMenu();
    Q_FOREACH(KoreUnits::Unit u, KoreUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(KoreUnits::name(u)), this);
...
{
    setText(KoreUnits::name(newUnits));
}
