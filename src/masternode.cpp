...
//     if (!forceCheck && (GetTime() - lastTimeChecked < Params().GetMasternodeCheckSeconds())) return;
//     lastTimeChecked = GetTime();

//     //once spent, stop doing the checks
//     if (activeState == MASTERNODE_VIN_SPENT) return;

//     if (!IsPingedWithin(Params().GetMasternodeRemovalSeconds())) {
//         activeState = MASTERNODE_REMOVE;
//         return;
//     }

//     if (!IsPingedWithin(Params().GetMasternodeExpirationSeconds())) {
//         activeState = MASTERNODE_EXPIRED;
//         return;
//     }

//     if (lastPing.sigTime - sigTime < Params().GetMasternodeMinMNPSeconds()) {
...
//     return Params().GetNetworkID() == CBaseChainParams::REGTEST ||
...
//             service.GetPort(), strService, nDefaultPort, Params().GetNetworkIDString());
...
//     if (pmn->pubKeyCollateralAddress == pubKeyCollateralAddress && !pmn->IsBroadcastedWithin(Params().GetMasternodeMinMNBSeconds())) {
...
//     // should be at least not earlier than block when 500 KORE tx got Params().GetMasternodeMinConfirmations()
//     uint256 hashBlock = 0;
//     CTransaction tx2;
...
//         CBlockIndex* pConfIndex = chainActive[pMNIndex->nHeight + Params().GetMasternodeMinConfirmations() - 1]; // block where tx got Params().GetMasternodeMinConfirmations()
...
//     if (Params().GetNetworkID() == CBaseChainParams::REGTEST) isLocal = false;
...
//         // last ping was more then Params().GetMasternodeMinMNPSeconds()-60 ago comparing to this one
//         if (!pmn->IsPingedWithin(Params().GetMasternodeMinMNPSeconds() - 60, sigTime)) {
