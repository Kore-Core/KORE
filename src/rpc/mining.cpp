...
UniValue generate(const UniValue& params, bool fHelp)
{
    ...
    {
        unique_ptr<CBlockTemplate> pblocktemplate(CreateNewBlock(Params(), coinbaseScript->reserveScript, NULL, false));
...
UniValue getstakinginfo(const UniValue& params, bool fHelp)
{
    ...
    if (staking)
        nExpectedTime = (cparams.nTargetSpacing * nNetworkWeight) / nWeight;
...
UniValue checkkernel(const UniValue& params, bool fHelp)
{
    ...
    CBlock *block = NULL;
    const Consensus::Params& cparams = Params().GetConsensus();
    unsigned int nBits = GetNextWorkRequired(pindexPrev, block, cparams, true);
	...
	pblocktemplate = CreateNewBlock(Params(), scriptDummy,pwalletMain, true);
	if (!pblocktemplate)
    ...
    result.push_back(Pair("blocktemplatesignkey", HexStr(scriptDummy)));
...
UniValue getmininginfo(const UniValue& params, bool fHelp)
{
    ...
    obj.push_back(Pair("currentblocksize", (uint64_t)nLastBlockSize));
    obj.push_back(Pair("currentblocktx",   (uint64_t)nLastBlockTx));
    obj.push_back(Pair("Work Difficulty",        GetDifficulty()));
    obj.push_back(Pair("Stake Difficulty",       GetDifficulty(GetLastBlockIndex(chainActive.Tip(), true))));
    obj.push_back(Pair("search-interval",      (int)nLastCoinStakeSearchInterval));
    obj.push_back(Pair("blockvalue",    (uint64_t)GetProofOfWorkSubsidy(chainActive.Tip()->nHeight+1,consensusParams)));
    obj.push_back(Pair("errors",           GetWarnings("statusbar")));
    obj.push_back(Pair("genproclimit",     (int)GetArg("-genproclimit", DEFAULT_GENERATE_THREADS)));
    obj.push_back(Pair("networkhashps",    getnetworkhashps(params, false)));
    obj.push_back(Pair("netstakeweight", GetPoSKernelPS()));
    obj.push_back(Pair("stake minimum weight",    (int64_t)nWeight));
    obj.push_back(Pair("stake maximum weight",    (int64_t)0));
    obj.push_back(Pair("combined weight",  (int64_t)nWeight));
    //obj.push_back(Pair("stakeinterest",    (int64_t)coinyearreward()));
    obj.push_back(Pair("pooledtx",         (int64_t)mempool.size()));
...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Koreis not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Koreis downloading blocks...");
        ...
        (mempool.GetTransactionsUpdated() != nTransactionsUpdatedLast && GetTime() - nStart > 5))
    {
        ...
        CScript scriptDummy = CScript() << OP_TRUE;
        pblocktemplate = CreateNewBlock(Params(), scriptDummy, NULL, false);
        if (!pblocktemplate)
            throw JSONRPCError(RPC_OUT_OF_MEMORY, "Out of memory");
