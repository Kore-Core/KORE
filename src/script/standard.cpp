...
bool Solver(const CScript& scriptPubKey, txnouttype& typeRet, vector<vector<unsigned char> >& vSolutionsRet)
{
    ...
    if (mTemplates.empty()) {
        ...
        mTemplates.insert(make_pair(TX_LOCKSTAKE, CScript() << Params().GetStakeLockSequenceNumber() << OP_CHECKSEQUENCEVERIFY << OP_DROP << OP_PUBKEY << OP_CHECKSIG));
    }
    ...
    BOOST_FOREACH (const PAIRTYPE(txnouttype, CScript) & tplate, mTemplates) {
        ...
        while (true) {
            ...
            if (opcode2 == OP_ANYDATA) {
                if (vch1.empty() && opcode1 != OP_0 && opcode1 < OP_1 && opcode1 > OP_16)
                    break;
                vSolutionsRet.push_back(vch1);
