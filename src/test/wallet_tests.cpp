...
static void add_coin(const CAmount& nValue, int nAge = 6*24, bool fIsFromMe = false, int nInput=0)
{
    ...
    if (fIsFromMe)
    {
        ...
        txIn.vout[0].scriptPubKey = scriptPubKey;
        CWalletTx* wtxIn = new CWalletTx(wallet, txIn);
        wallet->AddToWallet(*wtxIn, true, &wdb);
...
BOOST_AUTO_TEST_CASE(coin_selection_tests)
{
...

    {
        ...
        wallet->strWalletFile = "coin_selection_tests";
        CWalletDB walletDB(wallet->strWalletFile, "crw");
        static CPubKey key;
        wallet->GetKeyFromPool(key);
        scriptPubKey = GetScriptForDestination(key.GetID());
    }
    ...
    for (int i = 0; i < RUN_TESTS * 2; i++)
    {
        // Run the tests twice and change the height to fork in the middle
        if (i == RUN_TESTS - 1)
