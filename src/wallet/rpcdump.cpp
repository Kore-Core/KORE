...
void ImportAddress(const CKoreAddress& address, const string& strLabel);
void ImportScript(const CScript& script, const string& strLabel, bool isRedeemScript)
{
    ...
    if (isRedeemScript) {
        ...
        ImportAddress(CKoreAddress(CScriptID(script)), strLabel);
    }
}
...
UniValue importaddress(const UniValue& params, bool fHelp)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Koreaddress or script");
    }
...
UniValue importpubkey(const UniValue& params, bool fHelp)
{
    ...
    ImportAddress(CKoreAddress(pubKey.GetID()), strLabel);
...
UniValue importwalletfile(const UniValue& params, bool fHelp)
{
...

    {
        ...
        BOOST_FOREACH(const CKeyID &keyid, setKeys) {
            ...
            std::string strAddr = CKoreAddress(keyid).ToString();
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
    ...
    return CKoreSecret(vchSecret).ToString();
}
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        if (pwalletMain->GetKey(keyid, key)) {
            if (pwalletMain->mapAddressBook.count(keyid)) {
                file << strprintf("%s %s label=%s # addr=%s\n", CKoreSecret(key).ToString(), strTime, EncodeDumpString(pwalletMain->mapAddressBook[keyid].name), strAddr);
            } else if (setKeyPool.count(keyid)) {
                file << strprintf("%s %s reserve=1 # addr=%s\n", CKoreSecret(key).ToString(), strTime, strAddr);
            } else {
